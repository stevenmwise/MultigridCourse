MODULE Problem
USE Global
!
CONTAINS
!
FUNCTION Energy(u,f,m,h) RESULT(energyresult)
USE Global
IMPLICIT NONE
!
! Dirichlet Energy:
!
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: u
REAL(KIND=r8), DIMENSION(1:,1:), INTENT(IN):: f
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: m
REAL(KIND=r8), INTENT(IN):: h
REAL(KIND=r8):: energyresult
!
INTEGER:: i, j
INTEGER, DIMENSION(1:2):: mx
REAL(KIND=r8):: h2, tmp
REAL(KIND=r8), DIMENSION(0:SIZE(u,1)-2,1:SIZE(u,2)-2):: f1, m1
REAL(KIND=r8), DIMENSION(1:SIZE(u,1)-2,0:SIZE(u,2)-2):: f2, m2
!
CALL BoundaryConditions(u)
CALL BoundaryConditions(m)
!
mx(1) = SIZE(u,1)-2; mx(2) = SIZE(u,2)-2
!
h2 = h*h
tmp = 0.25_r8/h2
!
f1(0:mx(1),1:mx(2)) =  u(1:mx(1)+1,1:mx(2))-u(0:mx(1),1:mx(2))
m1(0:mx(1),1:mx(2)) = (m(1:mx(1)+1,1:mx(2))+m(0:mx(1),1:mx(2)))/2.0_r8
f1(0:mx(1),1:mx(2)) = m1(0:mx(1),1:mx(2))*f1(0:mx(1),1:mx(2))*f1(0:mx(1),1:mx(2))
!
f2(1:mx(1),0:mx(2)) =  u(1:mx(1),1:mx(2)+1)-u(1:mx(1),0:mx(2))
m2(1:mx(1),0:mx(2)) = (m(1:mx(1),1:mx(2)+1)+m(1:mx(1),0:mx(2)))/2.0_r8
f2(1:mx(1),0:mx(2)) = m2(1:mx(1),0:mx(2))*f2(1:mx(1),0:mx(2))*f2(1:mx(1),0:mx(2))
!
energyresult &
  =    h2*(-SUM( f(1:mx(1),1:mx(2))* u(1:mx(1)  ,1:mx(2)  )) &
  +     tmp*SUM(f1(1:mx(1),1:mx(2))+f1(0:mx(1)-1,1:mx(2)  ) &
  +             f2(1:mx(1),1:mx(2))+f2(1:mx(1)  ,0:mx(2)-1)))
!
END FUNCTION Energy
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
FUNCTION EnergyGradient(u,f,m,h) RESULT(energygradientresult)
USE Global
IMPLICIT NONE
!
! Dirichlet Energy Gradient (negative the residual vector):
!
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: u
REAL(KIND=r8), DIMENSION(1:,1:), INTENT(IN):: f
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: m
REAL(KIND=r8), INTENT(IN):: h
REAL(KIND=r8), DIMENSION(1:SIZE(u,1)-2, &
                         1:SIZE(u,2)-2):: energygradientresult
!
INTEGER, DIMENSION(1:2):: mx
!
CALL BoundaryConditions(u)
CALL BoundaryConditions(m)
!
mx(1) = SIZE(u,1)-2; mx(2) = SIZE(u,2)-2
!
energygradientresult(1:mx(1),1:mx(2)) = Operator(u(0:mx(1)+1,0:mx(2)+1), &
                                                 m(0:mx(1)+1,0:mx(2)+1),h) &
                                      -          f(1:mx(1)  ,1:mx(2)  )
!
END FUNCTION EnergyGradient
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
FUNCTION Operator(u,m,h) RESULT(operatorresult)
USE Global
IMPLICIT NONE
!
! Laplacian Operator:
!
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: u
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: m
REAL(KIND=r8), INTENT(IN):: h
REAL(KIND=r8), DIMENSION(1:SIZE(u,1)-2, &
                         1:SIZE(u,2)-2):: operatorresult
!
INTEGER, DIMENSION(1:2):: mx
REAL(KIND=r8):: h2
!
CALL BoundaryConditions(u)
CALL BoundaryConditions(m)
!
mx(1) = SIZE(u,1)-2; mx(2) = SIZE(u,2)-2
h2 = h*h
!
operatorresult(1:mx(1),1:mx(2)) = -(       u(2:mx(1)+1,1:mx(2)  ) &
                                  +        u(0:mx(1)-1,1:mx(2)  ) &
                                  +        u(1:mx(1)  ,2:mx(2)+1) &
                                  +        u(1:mx(1)  ,0:mx(2)-1) &
                                  - 4.0_r8*u(1:mx(1)  ,1:mx(2)  ))/h2
!
END FUNCTION Operator
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE Relaxation(u,f,m,h,passes)
USE Global
IMPLICIT NONE
!
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: u
REAL(KIND=r8), DIMENSION(1:,1:), INTENT(IN):: f
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: m
REAL(KIND=r8), INTENT(IN):: h
INTEGER, INTENT(IN):: passes
!
INTEGER:: i, its, j
INTEGER, DIMENSION(1:2):: mx
REAL(KIND=r8):: h2, omega2, tmp
!
mx(1) = SIZE(u,1)-2; mx(2) = SIZE(u,2)-2
h2 = h*h
omega2 = 1.0_r8-omega
!
CALL BoundaryConditions(u)
CALL BoundaryConditions(m)
!
DO its = 1, 2*passes
!
DO j = 1, mx(2)
  DO i = 1+MODULO(j+its,2), mx(1), 2
!
    tmp = (h2*f(i,j)+u(i+1,j  )+u(i-1,j  ) &
       +             u(i  ,j+1)+u(i  ,j-1))/4.0_r8
    u(i,j) = omega*tmp+omega2*u(i,j)
!
  END DO
END DO
!
CALL BoundaryConditions(u)
CALL BoundaryConditions(m)
!
END DO
!
END SUBROUTINE Relaxation
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
SUBROUTINE BoundaryConditions(u)
USE Global
IMPLICIT NONE
!
REAL(KIND=r8), DIMENSION(0:,0:), INTENT(IN OUT):: u
!
INTEGER, DIMENSION(1:2):: mx
!
mx(1) = SIZE(u,1)-2; mx(2) = SIZE(u,2)-2
!
! Periodic boundary conditions: Order is important!
u(      0,1:mx(2)) = u(mx(1),1:mx(2))
u(mx(1)+1,1:mx(2)) = u(    1,1:mx(2))
!
u(0:mx(1)+1,      0) = u(0:mx(1)+1,mx(2))
u(0:mx(1)+1,mx(2)+1) = u(0:mx(1)+1,    1)
!
END SUBROUTINE BoundaryConditions
END MODULE Problem