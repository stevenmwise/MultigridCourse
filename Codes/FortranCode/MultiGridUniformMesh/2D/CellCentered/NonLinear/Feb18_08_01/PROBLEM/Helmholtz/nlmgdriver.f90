...
REAL(KIND=r8):: c1, c2, tol, energyminimum, h, h1, h2, p1, p2, pi3, pi4, &
                piby2, r, x, y, z
...
END IF
!
ALLOCATE(u(  0:mx(1)+1,0:mx(2)+1))
ALLOCATE(f(  1:mx(1)  ,1:mx(2)  ))
ALLOCATE(res(1:mx(1)  ,1:mx(2)  ))
!
pi3 = pi*pi*pi
pi4 = pi*pi3
piby2 = pi/2.0_r8
...
DO i = 1, mx(1)
  ...
  DO j = 1, mx(2)
    ...
    z = (REAL(j,KIND=r8)-0.5_r8)*h+xlower(2)
!
!    r = SQRT((x-c1)**2+(y-c1)**2)
!
!    f(i,j) = EXP(-1.0E+01_r8*r*r)
!    f(i,j) = 2.0_r8*SIN((x-xlower(1))*pi/p1)*SIN((y-xlower(2))*pi/p2)*pi*pi
!    f(i,j) = 4.0_r8*pi*pi*pi*pi*SIN(pi*x)*SIN(pi*y)*(COS(pi*x)*COS(pi*x) &
!           - 3.0_r8*COS(pi*x)*COS(pi*x)*COS(pi*y)*COS(pi*y)+COS(pi*y)*COS(pi*y))
!
      f(i,j) = &
          ((pi4/4.0_r8)*COS(x*piby2)*COS(x*piby2)*COS(pi*SIN(x*piby2)) &
        +  (pi3/4.0_r8)*SIN(x*piby2)*SIN(pi*SIN(x*piby2)) &
        +  (pi4/4.0_r8)*COS(y*piby2)*COS(y*piby2)*COS(pi*SIN(y*piby2)) &
        +  (pi3/4.0_r8)*SIN(y*piby2)*SIN(pi*SIN(y*piby2)))
!
!
...
END DO
!
CALL NonLinearMultigrid(u,f,res,h,tol,iter,energyminimum)
!
CALL PrintOUT
!
DEALLOCATE(u,f,res)
